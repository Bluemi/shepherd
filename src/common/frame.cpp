...
constexpr float BUILD_RANGE = 5.f;
constexpr float DESTROY_RANGE = 5.f;
...
void frame::check_destroy_block(player* p) {
	if (p->poll_left_mouse_pressed()) {
		std::optional<world_block> block_to_destroy = blocks.get_colliding_block(ray(p->get_root_camera_position(), p->get_direction()), DESTROY_RANGE);
...
void frame::check_add_block(player* p) {
	if (p->poll_right_mouse_pressed()) {
		std::optional<glm::ivec3> block_placement_position = blocks.get_addition_position(ray(p->get_root_camera_position(), p->get_direction()), BUILD_RANGE);
